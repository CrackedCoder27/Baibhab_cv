...
BS-MS, Earth Sciences Major and Computer Science Minor \hfill CGPA: 8.34/10.00
...
\begin{itemize}
    ...
    \item Applying QSeek, a machine-learning-based framework for earthquake detection and localization, to continuous seismic data from the Kashmir Himalaya.

    \item Integrating neural-network-based phase pickers, including PhaseNet and EQTransformer from SeisBench, with stacking-migration and adaptive octree localization to develop a more comprehensive earthquake catalog for the region.

    \item Evaluating the QSeek-derived catalog against the existing regional catalog to assess improvements in detection completeness and location accuracy, and to better understand the spatial and temporal patterns of seismicity in the Kashmir Himalaya.
...
\begin{itemize}
    ...
    \item \textbf{Python Libraries:} NumPy, Pandas, SciPy, Matplotlib, ObsPy, Seaborn, Scikit-learn, SeisBench, TensorFlow and etc.
    \item \textbf{Geophysical Software:} SAC, SPECFEM2D, Pyrocko, SWRT, ASPECT(basic)
    \item \textbf{Machine Learning:} Supervised \& Unsupervised Learning (Regression, SVM, k-NN, Naive Bayes, Decision 
    Trees, k-Means, PCA, DBSCAN), 
    Ensemble Methods (Random Forest)
    \item \textbf{Deep Learning:} Neural Networks (Perceptron, MLP,), Optimization (Gradient Descent, Adam)
